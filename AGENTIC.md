...
- Clicking the hub icon opens the MCP configuration dialog (pairing, PSK, port) exactly like the code path described in the MCP section below.
...
- **LLM** connects to it over a secure channel (dual-mode authentication)
- **Tools** exposed to LLM include everything the app does, system remediations, network and traffic scanning, etc.

**Dual-mode authentication:**
1. **Per-client credentials** (app-mediated pairing): Desktop clients POST to `/mcp/pair`, user approves in the app, client receives an `edm_mcp_...` credential. No PSK copy/paste needed.
2. **Shared PSK** (CLI/headless): Legacy Bearer token for CLI tools, provisioning scripts, and automation. Unchanged from previous behavior.
...
- **Per-client credentials** (desktop apps): Client POSTs to `/mcp/pair`, user approves in the app, client receives `edm_mcp_...` credential. No PSK copy/paste.
- **Shared PSK** (CLI/headless): Legacy Bearer token for CLI tools, provisioning scripts, and automation. Unchanged.
...
2. **Choose authentication**
   - **Pairing (desktop clients):** Client initiates pairing; approve in app. Use pairing RPC methods: `mcpApprovePairing`, `mcpRejectPairing`, `mcpListPairedClients`, `mcpGetPendingPairingRequests`, `mcpRevokePairedClient`, `mcpRotatePairedClient`.
...
- **401 Unauthorized?**
  - Check credential is correct (paired `edm_mcp_...` or shared PSK; copy fresh from app if needed)
  - Verify format: `Authorization: Bearer <credential>` (note space after "Bearer")
